...
export default function Dashboard() {
    ...
    const [beneficiaryContracts, setBeneficiaryContracts] = useState([]);
    ...
    const getContractStatus = (contract) => {
        ...
        if (contract.triggered)
            return 'Triggered - Ready to claim';
        if (!contract.needsMonitoring)
            return 'Inactive';
    ...
    const getTimeRemaining = (contract) => {
        if (contract.triggered || contract.claimed)
            return 0;
        const timeSinceActivity = currentTime - contract.lastActivity;
    ...
    async function loadUserContracts() {
        ...
        try {
            ...
            setUserContracts(details.filter(Boolean));
        }
        catch (err) {
    ...
    async function loadBeneficiaryContracts() {
        ...
        try {
            ...
            const detailsPromises = contractAddresses.map(fetchContractDetails);
            const details = await Promise.all(detailsPromises);
            setBeneficiaryContracts(details.filter(Boolean));
        }
        catch (err) {
    ...
    async function createInheritanceContract() {
        if (!walletClient || !newBeneficiary || !newAmount)
            return;
        setCreating(true);
        try {
            const hash = await walletClient.writeContract({
                ...
                args: [newBeneficiary, BigInt(newInactivityTime)],
    ...
    // Claim inheritance
    async function claimInheritance(contractAddress) {
        if (!walletClient)
            return;
        try {
            const hash = await walletClient.writeContract({
    ...
    // Reset activity
    async function resetActivity(contractAddress) {
        if (!walletClient)
            return;
        try {
            const hash = await walletClient.writeContract({
    ...
    useEffect(() => {
        if (isConnected && (chain === null || chain === void 0 ? void 0 : chain.id) === CHAIN_ID) {
            refreshData();
        }
        else {
            setUserContracts([]);
    ...
    if ((chain === null || chain === void 0 ? void 0 : chain.id) !== CHAIN_ID) {
        return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
        <div className="flex items-center justify-center min-h-[60vh]">
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">      
      <div className="max-w-7xl mx-auto p-4 pt-20">
        ...
        <Card className={`mb-6 ${(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) ? 'border-green-200 bg-green-50' : 'border-orange-200 bg-orange-50'}`}>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Activity className={`h-5 w-5 ${(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) ? 'text-green-600' : 'text-orange-600'}`}/>
                <div>
                  <p className={`font-medium ${(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) ? 'text-green-800' : 'text-orange-800'}`}>
                    Monitoring Service: {(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) ? 'Active' : 'Inactive'}
                  </p>
                  <p className={`text-sm ${(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) ? 'text-green-600' : 'text-orange-600'}`}>
                    {(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning)
            ...
            : 'Monitoring service is not running. Contracts will not be automatically processed.'}
              ...
              <div className="flex gap-2">
                <Button onClick={startMonitoring} disabled={(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) || monitoringLoading} size="sm" className="flex items-center gap-2">
                  <Play className="h-4 w-4"/>
                  {monitoringLoading ? 'Starting...' : 'Start'}
                </Button>
                <Button onClick={stopMonitoring} disabled={!(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) || monitoringLoading} variant="outline" size="sm" className="flex items-center gap-2">
                  <Square className="h-4 w-4"/>
                  {monitoringLoading ? 'Stopping...' : 'Stop'}
                </Button>
                <Button onClick={checkMonitoringStatus} variant="ghost" size="sm" className="flex items-center gap-2">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            .slice(0, 5)
            .map((contract) => (<div key={contract.contractAddress} className="flex items-center justify-between">
                        <div>
          ...
          <TabsContent value="my-contracts" className="space-y-6">
            ...
            {userContracts.length === 0 ? (<Card>
                ...
                {userContracts.map((contract) => (<Card key={contract.contractAddress} className="relative">
                    <CardHeader>
                      <div className="flex items-center justify-between">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      {!contract.triggered && (<div className="flex flex-col gap-2">
                          <Button onClick={() => resetActivity(contract.contractAddress)} variant="outline" size="sm">
                            Reset Activity
                          </Button>
                          <Button onClick={() => emergencyWithdraw(contract.contractAddress)} variant="destructive" size="sm">
                            Emergency Withdraw
                          </Button>
                        </div>)}
                    </CardContent>
          ...
          <TabsContent value="beneficiary" className="space-y-6">
            ...
            {beneficiaryContracts.length === 0 ? (<Card>
                <CardContent className="pt-6 text-center">
                  <p className="text-muted-foreground">No beneficiary contracts found.</p>
                </CardContent>
              </Card>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {beneficiaryContracts.map((contract) => (<Card key={contract.contractAddress} className="relative">
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <CardTitle className="text-lg">
                          Contract {contract.contractAddress.slice(0, 8)}...
                    ...
                    <CardContent className="space-y-4">
                      ...
                      {contract.triggered && !contract.claimed && (<Button onClick={() => claimInheritance(contract.contractAddress)} className="w-full">
                          Claim Inheritance
                        </Button>)}
                    </CardContent>
                  </Card>))}
            ...
            <Card className="max-w-2xl mx-auto">
              ...
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  ...
                  <Input id="beneficiary" placeholder="0x..." value={newBeneficiary} onChange={(e) => setNewBeneficiary(e.target.value)}/>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="amount">Amount (tCORE2)</Label>
                  <Input id="amount" type="number" step="0.01" placeholder="1.0" value={newAmount} onChange={(e) => setNewAmount(e.target.value)}/>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="inactivity">Inactivity Time (seconds)</Label>
                  <Input id="inactivity" type="number" value={newInactivityTime} onChange={(e) => setNewInactivityTime(e.target.value)}/>
                  <p className="text-sm text-muted-foreground">
                    Current: {formatDuration(parseInt(newInactivityTime) || 0)}
                  </p>
                </div>

                <Button onClick={createInheritanceContract} disabled={creating || !newBeneficiary || !newAmount} className="w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <Badge variant={(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) ? 'default' : 'secondary'}>
                          {(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) ? 'Running' : 'Stopped'}
                        </Badge>
                        <span className="text-sm text-muted-foreground">
                          {(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) ? 'Actively monitoring contracts' : 'No active monitoring'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground font-mono">
                        {(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.walletAddress) || 'Not available'}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.lastCheck)
            ...
            beneficiaryContracts.filter(c => c.needsMonitoring).length}
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-2">
                        <Button onClick={startMonitoring} disabled={(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) || monitoringLoading} className="flex items-center gap-2">
                          <Play className="h-4 w-4"/>
                          {monitoringLoading ? 'Starting...' : 'Start Monitoring'}
                        </Button>
                        <Button onClick={stopMonitoring} disabled={!(monitoringStatus === null || monitoringStatus === void 0 ? void 0 : monitoringStatus.isRunning) || monitoringLoading} variant="outline" className="flex items-center gap-2">
                          <Square className="h-4 w-4"/>
                          {monitoringLoading ? 'Stopping...' : 'Stop Monitoring'}
                        </Button>
                        <Button onClick={checkMonitoringStatus} variant="ghost" className="flex items-center gap-2">
                          <RefreshCw className="h-4 w-4"/>
                          Refresh Status
                        </Button>
                      </div>
